...
void zend_compile_escaping_echo(zend_ast *ast) /* {{{ */
{
...

	{
		...
		zend_ast *class_name_ast, *method_name_ast, *args_ast, *call_ast;

		ZVAL_STRING(&class_name, "PHPEscaper");
		ZVAL_STRING(&method_name, "escape");

		class_name_ast = zend_ast_create_zval_ex(&class_name, ZEND_NAME_NOT_FQ);
		...
		call_ast = zend_ast_create(ZEND_AST_STATIC_CALL,
			class_name_ast, method_name_ast, args_ast
		);
